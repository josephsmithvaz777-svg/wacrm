-- ============================================================
-- 068_round_robin_order.sql
--
-- Lead rotation is no longer UUID order. Advisors with
-- profiles.round_robin_order go first (1, 2, 3…); everyone else
-- follows in user_id order — the "cuarto: otro asesor" slot.
-- ============================================================

ALTER TABLE profiles
  ADD COLUMN IF NOT EXISTS round_robin_order integer;

COMMENT ON COLUMN profiles.round_robin_order IS
  'Lower numbers receive auto-assigned leads first. Null = after numbered advisors.';
...
BEGIN
  ...
  SELECT COALESCE(
    ARRAY(
      ...
      ORDER BY round_robin_order NULLS LAST, user_id
    ),
    ARRAY[]::uuid[]
  )
  INTO v_pool;
