...
import { clampSilenceHandoffMinutes } from '@/lib/ai/silence-handoff'
...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(
        'provider, model, system_prompt, is_active, auto_reply_enabled, auto_reply_max_per_conversation, handoff_agent_id, silence_handoff_minutes, api_key, embeddings_api_key',
      )
...
export async function POST(request: Request) {
  try {
    ...
    const silenceHandoffMinutes = clampSilenceHandoffMinutes(
      body.silence_handoff_minutes,
    )
